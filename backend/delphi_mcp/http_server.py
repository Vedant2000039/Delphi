...
class DiscoverICPRequest(BaseModel):
    # No `product` field on purpose — ICP always uses the user's
    # already-saved product/service selection. Changing it happens
    # only via the sidebar product/service switcher, not this endpoint.
    user_id: int
    country_id: Optional[int] = None
    industry_id: Optional[int] = None
...
class DiscoverBuyerGroupsRequest(BaseModel):
    # No brand_id/brand_name field on purpose — the buyer group is
    # always scoped to the same brand(s) already matched from the
    # user's saved product during ICP discovery. Never a separate
    # user choice; resolved server-side from user_id.
    user_id: int
...
def discover_buyer_groups(payload: DiscoverBuyerGroupsRequest):
    ...
    try:
        result = svc.discover_buyer_groups(user_id=payload.user_id)
