...
There is intentionally no brand-options / brand picker endpoint
here anymore — the buyer group is always derived from the same
brand(s) already matched off the user's saved product during ICP
discovery (see BuyerGroupService.resolve_brand_context), so the
frontend only ever needs to send user_id.
"""
